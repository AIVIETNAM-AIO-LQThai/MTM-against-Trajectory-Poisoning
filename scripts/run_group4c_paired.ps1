param(
    [switch]$DryRun
)

$ErrorActionPreference = "Stop"

$jobs = @(
    @{
        Condition   = "canonical"
        Rho         = "0.01"
        RhoSlug     = "001"
        DatasetRoot = "data/poisoned/csdpc/walker2d-medium-v2"
    },
    @{
        Condition   = "canonical"
        Rho         = "0.05"
        RhoSlug     = "005"
        DatasetRoot = "data/poisoned/csdpc/walker2d-medium-v2"
    },
    @{
        Condition   = "s2_overlap_r0"
        Rho         = "0.01"
        RhoSlug     = "001"
        DatasetRoot = "data/poisoned/csdpc_s2_overlap_r0_v1/walker2d-medium-v2"
    },
    @{
        Condition   = "s2_overlap_r0"
        Rho         = "0.05"
        RhoSlug     = "005"
        DatasetRoot = "data/poisoned/csdpc_s2_overlap_r0_v1/walker2d-medium-v2"
    }
)

foreach ($job in $jobs) {
    foreach ($seed in 0, 1, 2) {

        $dataset = Join-Path `
            $job.DatasetRoot `
            "rho_$($job.RhoSlug)_seed_$seed.hdf5"

        $outDir = `
            "experiments/dt_stress/walker2d_medium/" +
            "$($job.Condition)/rho_$($job.RhoSlug)/" +
            "attack_seed_$seed/train_seed_$seed"

        $summary = Join-Path $outDir "summary.json"

        Write-Host ""
        Write-Host ("=" * 78)
        Write-Host "$($job.Condition) rho=$($job.Rho) attack=$seed train=$seed"
        Write-Host ("=" * 78)

        if (-not (Test-Path $dataset)) {
            throw "Dataset missing: $dataset"
        }

        # Already completed -> never overwrite.
        if (Test-Path $summary) {
            $savedSummary = Get-Content $summary -Raw | ConvertFrom-Json

            if (
                $savedSummary.status -eq "complete" -and
                [int]$savedSummary.final_step -eq 100000
            ) {
                Write-Host "SKIP: already complete at 100000."
                continue
            }
        }

        $latestCheckpoint = $null
        $checkpointDir = Join-Path $outDir "checkpoints"

        if (Test-Path $checkpointDir) {
            $latestCheckpoint = Get-ChildItem `
                $checkpointDir `
                -Filter "checkpoint_step_*.pt" `
                -File `
                -ErrorAction SilentlyContinue |
                Sort-Object Name -Descending |
                Select-Object -First 1
        }

        $args = @(
            "-m", "scripts.train_dt_stress",
            "--dataset", $dataset,
            "--condition", $job.Condition,
            "--rho", $job.Rho,
            "--attack-seed", "$seed",
            "--seed", "$seed",
            "--num-updates", "100000",
            "--checkpoint-every", "10000",
            "--output-dir", $outDir
        )

        if ($latestCheckpoint) {
            Write-Host "RESUME: $($latestCheckpoint.FullName)"
            $args += @(
                "--resume",
                $latestCheckpoint.FullName
            )
        }
        elseif (Test-Path $outDir) {
            $existingFiles = @(
                Get-ChildItem `
                    $outDir `
                    -Recurse `
                    -File `
                    -ErrorAction SilentlyContinue
            )

            if ($existingFiles.Count -gt 0) {
                throw @"
Incomplete run exists but has no checkpoint:
$outDir

Inspect/delete it manually rather than overwriting it.
"@
            }
        }

        if ($DryRun) {
            Write-Host "DRY RUN:"
            Write-Host "python $($args -join ' ')"
            continue
        }

        & python @args

        if ($LASTEXITCODE -ne 0) {
            throw "Training failed for $($job.Condition), rho=$($job.Rho), seed=$seed"
        }
    }
}
